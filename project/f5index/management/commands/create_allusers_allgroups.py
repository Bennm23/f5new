...
class Command(BaseCommand):
    help = 'Create users, assign them to groups, and assign a random profile picture'
    ...
    def handle(self, *args, **options):
        ...
        all_profile_pictures = list(ProfilePicture.objects.all())

        # Function to create a user and assign a random profile picture
        def create_user(username, email, password, group, bio, is_superuser=False):
            if not all_profile_pictures:
                raise ValueError("No profile pictures available")

            if is_superuser:
                user = Member.objects.create_superuser(
                    username=username,
                    email=email,
                    password=password,
                    is_verified=True
            ...
            random_profile_picture = random.choice(all_profile_pictures)
            user.profile = random_profile_picture  # Assuming Member model has a profile field
            user.save()

            group.user_set.add(user)
            self.stdout.write(self.style.SUCCESS(f'Successfully created {"superuser" if is_superuser else group.name.lower()}: {user} with profile picture: {random_profile_picture.name}'))

        # Fetch superuser credentials from .env file or use default
        ...
        # Create and add the superuser
        create_user(superuser_username, superuser_email, superuser_password, staff_group, 'Superuser bio', is_superuser=True)

        # Create and add the other users
        create_user('player_username', 'player@example.com', 'player_password', player_group, 'Short bio of the player')
        create_user('coach_username', 'coach@example.com', 'coach_password', coach_group, 'Short bio of the coach')
